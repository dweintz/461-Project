...
def test_dataset_type_repo():
    with patch("src.scorer.metrics.dataset_and_code.HF_API") as mock_hf_api_instance, \
         patch("src.scorer.metrics.dataset_and_code.get_repo_id", return_value="mock/dataset"):
        ...
        from src.scorer.metrics.dataset_and_code import get_dataset_and_code_score
        score, latency = get_dataset_and_code_score("https://huggingface.co/mock/dataset", "dataset")
        assert score == 0.0
...
def test_invalid_url_type():
    from src.scorer.metrics.dataset_and_code import get_dataset_and_code_score
    score, latency = get_dataset_and_code_score("https://huggingface.co/mock/repo", "invalid_type")
    assert score is None
    assert isinstance(latency, int)

def test_repo_id_exception():
    with patch("src.scorer.metrics.dataset_and_code.get_repo_id", side_effect=Exception("bad URL")):
        from src.scorer.metrics.dataset_and_code import get_dataset_and_code_score
        score, latency = get_dataset_and_code_score("https://huggingface.co/mock/repo", "model")
        assert score is None
        assert isinstance(latency, int)