...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "url_file",
        type=Path,
        help="Path to a newline-delimited file containing URLs"
    )
    parser.add_argument("--parallel", action="store_true", help="Calculate metrics in parallel")
    parser.add_argument("--log-file", type=Path, default=None, help="Path to write log file")
    parser.add_argument(
        "--log-level", type=int, choices=[0,1,2], default=int(os.environ.get("LOG_LEVEL","0")),
        help="Verbosity: 0=silent, 1=info, 2=debug"
    )
    parser.add_argument("--log-text", action="store_true", help="Use plain text logs instead of JSON lines")
    parser.add_argument("--run-id", default=None, help="Optional run id")
    return parser.parse_args()

def read_urls(file_path: Path) -> list[str]:
    """Read URLs line by line, stripping whitespace and ignoring empty lines."""
    if not file_path.exists():
        raise FileNotFoundError(f"URL file {file_path} does not exist.")
    urls = []
    with file_path.open("r", encoding="utf-8") as f:
...
def main() -> None:
    ...
    try:
        url_lines = read_urls(args.url_file.resolve())
        log.info("read urls", extra={"phase":"run","count":len(url_lines)})
    except Exception as e:
        log.exception("failed to read url file", extra={"phase":"run"})
        print(f"Error reading URL file: {e}", file=sys.stderr)
        sys.exit(1)

    # Process each line individually (1 line = 1 NDJSON object)
    for line in url_lines:
        ...
        name = ""
        category = ""
        net_score = ramp_up = bus_factor = performance_claims = license_score = dataset_and_code_score = dataset_quality = code_quality = 0.0
        net_score_latency = ramp_up_latency = bus_factor_latency = performance_claims_latency = license_latency = size_latency = dataset_and_code_score_latency = dataset_quality_latency = code_quality_latency = 0
        size_dict = {"raspberry_pi":0.0, "jetson_nano":0.0, "desktop_pc":0.0, "aws_server":0.0}
        ...
        for url in urls:
            try:
                url_type = classify_url(url)
                if url_type == "unknown":
                    raise ValueError(f"Unknown URL type for {url}")
            except Exception:
                log.exception("classification failed", extra={"phase":"controller"})
                print(f"Error: Unknown URL type for {url}", file=sys.stderr)
                sys.exit(1)

            # Take first model URL for the name and category
            if url_type == "model" and not name:
                name = url.split("/")[-1]
                category = "MODEL"
